...
- **Privacy** — local-first: no accounts, no first-party cloud, no telemetry. The `INTERNET` permission ships with the AI features (Settings → AI Service: bring your own API key and configure your provider's endpoint; the app talks directly to the provider you configure). Until you configure an AI service, the app makes no network requests at all. WebDAV backup/restore (self-hosted) is still planned.
