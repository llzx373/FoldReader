...
/**
 * 选择状态：锚点（按下点）+ 当前光标（拖动点）。
 * 注：跨页选择降级为"当前页内选择"（见 TODO 5.2 注记），start/end 始终落在同一页。
 */
data class SelectionUi(
    val pageLeft: Boolean,
    val anchor: Long,
    val caret: Long,
...
fun handlePosition(boxes: List<LineBox>, offset: Long): Offset? {
    ...
    val x = box.boundaryX((offset - box.line.charStart).toInt())
    return Offset(x, box.yBottom)
}

/**
 * 选择手柄覆盖层：起/止两枚圆点，可拖动调整选区。
...
fun SelectionHandles(
    ...
    scrollYPx: Float,
    accent: Color,
    onDragHandle: (isStart: Boolean, pageLocal: Offset) -> Unit,
) {
    val density = LocalDensity.current
    val touchPx = with(density) { 28.dp.toPx() }
    val halfLine = boxes.firstOrNull()?.lineHeightPx?.div(2f) ?: 0f

    listOf(true to start, false to end).forEach { (isStart, offset) ->
        val pos = handlePosition(boxes, offset) ?: return@forEach
                ...
                .pointerInput(pos) {
                    detectDragGestures(
                        ...
                        onDrag = { change, dragAmount ->
                            ...
                            onDragHandle(isStart, Offset(dragCenter.x, dragCenter.y - halfLine))
                        },
