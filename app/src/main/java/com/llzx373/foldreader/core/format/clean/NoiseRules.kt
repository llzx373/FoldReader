...
internal object NoiseRules {
    ...
    private val RULES: List<Rule> = listOf(
        ...
        // ── 作者的话 / 求票 ─────────────────────────────────────────
        // 刻意**不含裸的「作者：」**：文件头的 `作者：1062137287` 是元数据，删掉就丢作者了。
        // 宁可漏掉章节里的某句「作者：…」，也不能删真实信息。
        Rule(
            "author-note",
            Regex("^\\s*[【\\[（(]?\\s*(?:作者的话|公告|通知|说明|ps|p\\.s\\.|ps\\.)\\s*[】\\]）)]?\\s*[:：].*$", RegexOption.IGNORE_CASE),
        ),
        ...
        // ── 装饰线 / 分隔线 ─────────────────────────────────────────
        // 允许行内空白：`----- -----` 这种「断成两截」的分隔线很常见，漏掉的话会被标点规则
        // 变成 `—— ——`，反倒像正文里多了一句破折号。
        Rule("decoration", Regex("^\\s*(?:[\\-=*_~☆★※◇◆■□●○▽▼△▲#+·]\\s*){3,}$")),
        Rule(
            "decoration",
            Regex("^\\s*(?:[☆★※◇◆■□●○]\\s*){3,}$"),
        ),
