...
### 13.2 人物出场索引
- [ ] 纯规则人名统计：2-4 字专名候选 + 称谓词过滤 + 频次阈值（`core/format/` 内，纯 JVM + 单测）
- [ ] 首次出场章节计算与落库（随章节索引流程产出）
- [ ] 目录面板「人物」页签：人物 → 首次出场章节 → 点击跳转
- [ ] 反向用例：常见虚词/称谓不被误判为人名
...
- [ ] 疑似标题行候选集粗筛（复用现有章节扫描器，几百行采样）
- [ ] prompt：返回 1~3 条候选正则 + 设计说明；解析失败丢弃该候选
- [ ] 本地逐条真实切分并预览：章节数、前 20 个标题、异常标记（超长章/空章/孤儿文本）
- [ ] 用户选定 → 存为该书的自定义章节规则（复用现有结构）→ 重扫目录
- [ ] 入口：目录面板「未识别到章节」状态 + 书籍详情页
