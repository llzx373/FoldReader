...
- [ ] TXT 自动封面生成：书名首字 + 书脊式配色（按书名哈希取色），Expressive 圆角形状
- [ ] 封面下显示阅读进度百分比与最近阅读时间；按最近阅读排序
- [ ] FAB Menu 导入口（Expressive `FloatingActionButtonMenu`）
- [ ] 可折叠大标题 TopAppBar；空书架引导页（插画位 + "导入书籍"按钮）
- [ ] 长按多选 → 删除（含确认对话框，可选"同时删除本地进度/标注"）
- [ ] 书架项点击进入阅读器（占位路由即可，M2 实现正文）
...
- [ ] `LayoutConfig` 模型：字号、行距、字距、段落间距、页边距（四边独立）、首行缩进、对齐方式、字体
- [ ] `Paginator`：输入文本窗口 + 页面像素尺寸 + LayoutConfig，基于 `StaticLayout` 度量输出 `Page(charStart, charEnd, lines)`
- [ ] 中文排版规则：标点避头尾（禁则处理）、两端对齐、首行缩进 2 字符
- [ ] 分页缓存：key = bookId + LayoutConfig 哈希 + 页面尺寸，内存 LRU + 磁盘缓存
- [ ] 双向翻页：支持"从偏移向后排一页"与"向前倒推一页"（上一页定位）
- [ ] 行长上限约束：中文每行 18–40 字，超出屏宽时居中留白（为展开态横持铺垫）
- [ ] 单元测试：边界字符（标点行首/行尾）、窗口边界翻页、改字号后页数变化

### 2.2 单页阅读界面（feature/reader）
- [ ] 阅读页 Compose 渲染：按 Page 行布局绘制文本（Canvas/多 Text 行），全屏沉浸（隐藏系统栏，点击呼出）
- [ ] 翻页交互：点击热区（左/右/下翻页，热区比例可配）+ 水平滑动手势 + 音量键翻页（可关）
- [ ] 翻页动画 v1：覆盖滑动 + 无动画两种（仿真翻页在 M4）
- [ ] 上下滚动模式（RecyclerView 等效 LazyColumn 文本流，与分页模式互斥可切换）
- [ ] 阅读菜单：底部 Expressive 面板（目录/进度/亮度/设置入口），顶部栏（书名/章节/返回）
- [ ] 页眉页脚：章节名、页码/百分比、电量、时间（均可单独开关）
- [ ] 屏幕常亮开关、应用内独立亮度（窗口亮度 + 下拉手势调节）

### 2.3 进度与目录
- [ ] 进度锚点 = 当前页首字符偏移，翻页即写（防抖落库）
- [ ] 打开书籍恢复到上次位置（锚点 → 重分页 → 定位）
- [ ] 进度条跳转（拖动百分比）、章节跳转
- [ ] 目录面板：章节列表、当前章节高亮、点击跳转；无目录书显示"按百分比"兜底
- [ ] 全书剩余时间估算（按近期阅读速度）
- [ ] 阅读时长统计埋点（前后台切换暂停计时）

### 2.4 阅读设置
- [ ] 字号/行距/边距滑杆（带刻度停靠 Slider），实时预览生效
- [ ] 字体：系统字体列表 + 导入 TTF/OTF
- [ ] 阅读主题预设 ≥5：护眼绿、羊皮纸、灰白、夜间、AMOLED 纯黑；自定义背景色/文字色
- [ ] 夜间模式：随系统/手动；阅读主题独立于系统动态取色
- [ ] 翻页方式、热区、音量键、常亮等开关接入 DataStore（全局默认）+ 单书覆盖
...
### 3.1 双页书式模式（说明书 5.2）
- [ ] FLAT + 垂直铰链 + 宽度足够时进入双页：左屏=左页、右屏=右页，一次翻两页
- [ ] 铰链避让：按 `hingeBounds` 将屏幕分为左右安全区，正文严格避开折痕
- [ ] 书脊设计：中缝柔和阴影渐变；内侧（书脊侧）页边距 > 外侧，模拟装订
- [ ] 页眉页脚分居左右页外侧；页码以"第 x/y 页（双页计）"呈现
- [ ] 阔折叠适配：宽内屏双页时行长取舒适区间下限；所有尺寸以运行时 FoldingFeature 实测，不写死
- [ ] 设置项：展开态可强制单栏（宽幅单栏，行长上限居中）
- [ ] 横持展开态保持双页，行长上限约束生效

### 3.2 姿态连续流转（说明书 3.3）
- [ ] 折叠↔展开切换：以锚点重分页并定位，单页↔双页无感切换，不闪白屏（切换期间保留上一帧渲染）
- [ ] HALF_OPENED 姿态识别接入阅读器（M4 铺 UI，本里程碑先保证状态流正确驱动布局）
- [ ] 旋转切换不丢进度
- [ ] 回归测试：连续折叠/展开 20 次进度锚点不变

### 3.3 双页性能
- [ ] 双页翻页预渲染（前后各一对页离屏就绪）
- [ ] 展开态 60fps（高刷屏 120fps）profile 验证，掉帧优化（减少重组、文本绘制缓存）
...
### 4.1 悬停桌面模式（说明书 5.3）
- [ ] HALF_OPENED + 水平铰链：上半屏正文（单栏），下半屏控制面板
- [ ] 下半屏面板：翻页按钮、进度条/章节跳转、亮度、字号快捷调节、自动翻页开关与速度
- [ ] 下半屏熄屏选项（面板区变暗，AMOLED 节能）
- [ ] 竖铰 HALF_OPENED（左右开合半折）：降级为单页 + 避让铰链
