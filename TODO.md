...
### M12.5 真实文件实跑修复（已完成）
> 用一本 296KB / 5601 行 / CRLF / UTF-8 BOM / 3 篇拼接的网文合集 TXT 实跑，
> 暴露出一批问题，逐条修掉并各自补了语料用例。
> （样例文件不入库，也不在文档里留文件名。）
- [x] 缩进分段的书里，**句末标点之后的续行没并回**（`『…』` + `他沉默着没有回答`）——
      漏并上百处。判据改为分两条路：这一段用缩进起头时，「下一行没有缩进且中间无空行」
      即权威续行信号，标点不作数
...
- [x] 夹空格的分隔线（`----- -----`）漏网后被变成 `—— ——` → 装饰线允许行内空白
- [x] 文件头的 `作者：xxx` 被当广告行删掉 → 作者行规则不再匹配裸「作者：」
- [x] 关闭「字符归一」时全角缩进不被认作缩进 → 缩进判定与空白规则统一
- [x] 实测结果：三级档位**全部幂等**；「无缩进的正文行」从 1637 降到 10
      （余下的是书名、作者行、分篇标题与一段本就顶格的对话记录）
