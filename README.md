...
- **网络仅用于用户显式配置的功能**：当前版本 `AndroidManifest.xml` 不声明任何权限、不产生任何网络请求。规划中的两类功能会引入 `INTERNET` 权限——AI 功能（用户自带 API key，直连其配置的服务商）与 WebDAV 备份/恢复（用户自托管服务器）。未配置这些功能时，应用不会有任何网络行为。
